...
- Prefer `GEMINI_MODEL` if set. Otherwise the scripts default to `gemini-flash-latest`.
...
4. Send a short progress message before ROI selection:
- `I am trying to find some area that worth to go high power.`

5. Ask Gemini to propose ROIs from the thumbnail:

```bash
/opt/pathclaw-venv/bin/python /home/node/.claude/skills/wsi-analysis/scripts/propose_rois_with_gemini.py \
...
6. Render each proposed ROI with `mcp__pathclaw_wsi__render_wsi_region` using the `level0_region` values from the JSON.

7. Build two artifacts:
...
8. Send the annotated thumbnail with ROI boxes to the user.

9. Send a text message that explains why each ROI was chosen.

Required pattern:
- `ROI_1: ...`
- `ROI_2: ...`
- one line per ROI using Gemini's rationale from `roi-proposals.json`

10. For each ROI, do the following in order:
- send the ROI image to the user
- ask Gemini to analyze that ROI
- send Gemini's thought / interpretation back to the user before moving to the next ROI

The user should see every step. Do not wait until all ROIs are done before sending updates.

11. After all ROI-level messages are sent, send the annotated thumbnail and ROI panel to Gemini for whole-case analysis:

```bash
/opt/pathclaw-venv/bin/python /home/node/.claude/skills/wsi-analysis/scripts/analyze_wsi_with_gemini.py \
...
12. Send the final structured summary to the user.
...
5. ROI_1 image
6. ROI_1 Gemini thought
7. ROI_2 image
8. ROI_2 Gemini thought
