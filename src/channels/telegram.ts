...
import { Channel, OutboundMessage, RegisteredGroup } from '../types.js';
...
export class TelegramChannel implements Channel {
  ...
  async sendMediaMessage(jid: string, message: OutboundMessage): Promise<void> {
    if (!this.bot) {
      logger.warn('Telegram bot not initialized');
      return;
    }
    ...
    for (const part of message.parts) {
      try {
        if (part.type === 'text') {
          await this.sendMessage(jid, part.text);
          continue;
        }

        if (!fs.existsSync(part.path)) {
          throw new Error(`Image file not found: ${part.path}`);
        }

        // Use a read stream to force multipart upload. Passing a plain path
        // has been observed to hit Telegram's URL/file_id code path instead.
